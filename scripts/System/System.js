...
client.on('OnChat', chat => {
    ...
    const censor = (msg) => {
        ...
        const filter = new RegExp(cwData.censorWord.join('|'), 'gi'); //filter censor word from message
        return msg.replace(filter, convert); //replace censor word with char
    };
    let check = false; //check profanity
    const cmd = client.commands.get('censorword').options; //get command option
